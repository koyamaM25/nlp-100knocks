...
NUM_LAYERS = 1
BIDIRECTIONAL = False
DROPOUT_P = 0.0   # LSTMのdropoutは num_layers>=2 で効く。必要なら classifier側でDropoutしてもOK。
...
class LSTMClassifier(nn.Module):
    def __init__(
        ...
        # LSTM（batch_first=True で (B,L,D) 入力）
        self.lstm = nn.LSTM(
            input_size=emb_dim,
            ...
            dropout=dropout_p if num_layers >= 2 else 0.0
        )
    ...
    def forward(self, input_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        # PAD部分を無視してLSTMに入れる（pack）
        # lengths は CPU の方が安全なことが多い
...
def evaluate(model: nn.Module, loader: DataLoader, criterion: nn.Module) -> Dict[str, float]:
    ...
    for batch in loader:
        ...
        logits = model(input_ids, lengths)  # (B,1)
...
def main():
    ...
    # 保存（必要なら）
    save_path = os.path.join(os.path.dirname(TRAIN_PKL_PATH), "out_76model_lstm.pt")
